    ...
    class InvitationsController < ApplicationController
      ...
      def send_pass_invitation_code
        ...
        message = @client.account.messages.create(
          :body => "#{code} is your verification code for Pass!",
          :to => params[:number],
          :from => "+16473605128"
        )
        if message
      ...
      def resend_invitation
        ...
        if NotificationsMailer.invitation_email(params[:email], @organization[:name], @invitation).deliver
          ...
          render json: { "eXpresso" => { "code" => 1, "message" => "Invitation failed." } }
        end
      end

      def manage_invitee
          @invitation[:location] = params[:location] if params[:location].present?
          @invitation[:user_group] = params[:user_group] if params[:user_group].present?
          @invitation[:phone_number] = params[:number] if params[:number].present?
          if @invitation.save!
               render json: { "eXpresso" => { "code" => 1, "message" => "Invitee update success." } }
          else
               render json: { "eXpresso" => { "code" => -125, "message" => "Invitee update failed." } }
          end
      end

      def manage_group_invitee
        if params[:locations].present?
          if Invitation.where(:id => params[:ids]).update_all(:location => params[:locations])
            render json: { "eXpresso" => { "code" => 1, "message" => "Invitee bulk update success." } }
          else
            render json: { "eXpresso" => { "code" => -140, "message" => "Invitee bulk update failed." } }
          end
        elsif params[:user_groups].present?
          if Invitation.where(:id => params[:ids]).update_all(:user_group => params[:user_groups])
            render json: { "eXpresso" => { "code" => 1, "message" => "Invitee bulk update success." } }
          else
            render json: { "eXpresso" => { "code" => -141, "message" => "Invitee bulk update failed." } }
          end
        end
      end
      ...
      def seek_domain
        domain = params[:data]
        if safelist(domain) || WhitelistedDomain.exists?(:domain => domain)
      ...
      def safelist(domain)
        ...
        if list.include?(domain)
          #Rails.logger.debug("in list")
          true
        else
          #Rails.logger.debug("not in list")
          false
        end
      end
