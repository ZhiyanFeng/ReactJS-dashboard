  ...
  module Bumblebee
    class ImagesController < ApplicationController
      ...
      def create
        ...
        if @image.save
          render json: @image.id, status: :created
      ...
      def drop_profile
        @image = Image.new(:org_id => params[:org_id], :owner_id => params[:owner_id])
        
        if is_equal_to(params[:reference], "org_profile")
          if @image.create_upload_and_set_organization_profile(params[:org_id], params[:file])
            render json: @image, serializer: ImageSerializer
          else
            render json: @image.errors
          end
      ...
      def upload_image
        ...
        if is_equal_to(params[:reference], "user_profile")
          ...
          if @image.create_and_upload_organization_cover_image(params[:org_id], params[:file])
            render json: @image, serializer: ImageSerializer
          else
            render json: @image.errors
          end
