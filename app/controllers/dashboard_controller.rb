class DashboardController < ApplicationController
  ...
  before_filter :set_basics, :except => [:select, :cookies_expires, :pick, :selected]

  def set_basics
    if cookies[:org_id].present?
      @organization = Organization.find(cookies[:org_id])
      @user = User.find(cookies[:user_id])
      @profile_image = []
      if @organization[:profile_id].present?
        @profile_image = Image.find(@organization[:profile_id])
        @profile_thumb = @profile_image.thumb_url
        @profile_full = @profile_image.full_url
      else
        #@profile_thumb = "http://www.coffeemobile.com/wp-content/uploads/2014/08/zFX6kVyD_400x400.png"
        #@profile_full = "http://www.coffeemobile.com/wp-content/uploads/2014/08/zFX6kVyD_400x400.png"
        @profile_thumb = "https://s3.amazonaws.com/coffeemobileassets/profile.png"
        @profile_full = "https://s3.amazonaws.com/coffeemobileassets/profile.png"
      end
      ...
      if @organization.unit_number.present?
        address_line = address_line + @organization.unit_number + " - "
      end
      if @organization.street_number.present?
        address_line = address_line + @organization.street_number + " "
      end
      if @organization.address.present?
        address_line = address_line + @organization.address + " "
      end

      if @organization.city.present?
          locale_line = locale_line + @organization.city + ", "
      end
      if @organization.province.present?
          locale_line = locale_line + @organization.province + " "
      end

      if @organization.postal.present?
          postal_line = postal_line + @organization.postal + ", "
      end
      if @organization.country.present?
          postal_line = postal_line + @organization.country + " "
      end

      @org_address = "<address>" + address_line + "<br />" + locale_line + "<br />" + postal_line + "</address>"
    else
      redirect_to login_url
      flash[:notice] = "Please login before trying to access the dashboard."
    end
  end
  ...
  def select
    ...
    if @keychain.size > 0
      if @keychain.first[:is_system]
        cookies[:is_system_user] = true
        ...
        #redirect_to shome_url


        #render :layout => false
        render "dashboard/pick", :layout => 'sessions'
  	    ...
  	    #@keychain = "hello"
  		  ...
  		  @apikey = ApiKey.new(:app_platform => "Dashboard", :app_version => "3.3.1")
  		  @apikey.save
			  cookies[:org_id] = @keychain.first[:org_id]
  			cookies[:api_key] = @apikey[:access_token]
  		  @organization = Organization.find(cookies[:org_id])
  ...
  def home
    ...
    if @org_age < 90
      @start_date = @organization[:created_at]
    else
      @start_date = nighty_days_ago
    end
    @end_date = DateTime.now
    #@post_count = Post.all(:conditions => {:org_id => @organization[:id]}).count
    @post_count = Post.where("org_id = ? AND post_type IN (?) AND is_valid", 
      @organization[:id], 
      PostType.reference_by_base_type("post")
    ).count

    #@user_count = User.all(:conditions => {:active_org => @organization[:id]}).count
  ...
  def system_home
    @data ||= Array.new
    @org = Organization.find(:all, :order => 'created_at asc').first
    @org_age = (DateTime.now.to_i - @org[:created_at].to_i)/24/60/60
    @start_date = @org[:created_at]

    @end_date = DateTime.now
    #@post_count = Post.all(:conditions => {:is_valid => true}).count
    @post_count = Post.where("org_id = ? AND post_type IN (?) AND is_valid", 
      @organization[:id], 
      PostType.reference_by_base_type("post")
    ).count
    @user_count = User.where(:is_valid => true, :validated => true).where.not(:active_org => 0).count
    
    query = 'SELECT "like".* FROM "likes" AS "like" FULL JOIN "users" 
    AS "user" ON "like"."owner_id"="user"."id" WHERE "user"."active_org" > 0 
    AND "user"."is_valid" AND "like"."is_valid"'
    #Rails.logger.debug(query)
    connection = ActiveRecord::Base.connection()
    @like_count = connection.execute(query).count
    ...
    query = 'SELECT "message".* FROM "chat_messages" AS "message" FULL JOIN "chat_sessions" 
    AS "cookies" ON "message"."session_id"="cookies"."id" WHERE "cookies"."is_valid" AND "message"."is_valid"'
    connection = ActiveRecord::Base.connection()
    @message_count = connection.execute(query).count
  end
  ...
  def announcements
    ...
    #@groups ||= Array.new
    @groups = UserGroup.where(:org_id => @organization[:id])
    group = {
      :id => 0, 
      :member_count => 0,
      :group_name => "All",
    ...
    #@locations ||= Array.new
    @locations = Location.where(:org_id => @organization[:id])
    location = {
      :id => 0, 
      :member_count => 0,
      :location_address => "",
  ...
  def settings
    ...
    @organization = Organization.find(cookies[:org_id])
    @user_groups = UserGroup.where(:org_id => cookies[:org_id], :is_valid => true)
    @locations = Location.where(:org_id => cookies[:org_id], :is_valid => true)
  ...
  def get_environment_variables
    # return a JSON file with the environment variables for use within the javascript
    render json: { "eXpresso" => { "code" => 1, "message" => "Get environment variables success.", "response" => ENV_VARS} }
  end
  # END DAVID ADDED
