class SyncSystemSubscriptionSerializer < ActiveModel::Serializer
  ...
  def content_since_last_refresh
    @post = Post.where("channel_id = #{object.channel_id} AND post_type IN (5,6,7,8,9,1,2,3,4,10,21) AND owner_id != #{user_id} AND created_at > TIMESTAMP '#{object.params_sync_time}' AND is_valid")
    @post.count
  end
