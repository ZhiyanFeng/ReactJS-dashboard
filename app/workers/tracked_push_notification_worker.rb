class TrackedPushNotificationWorker
  ...
  def perform(user_id,user_first_name,user_last_name,user_push_to,user_push_id,post,cpr_id,post_archtype,base_type)
    response = 0
    if post_archtype
      message = "#{user_first_name} #{user_last_name} posted a shift trade request. Interested in helping out?"
      response = Mession.tracked_subscriber_push("open_app", message, 4, post[:id], user_id, post[:channel_id], user_push_to, user_push_id)
      ...
      response = Mession.tracked_subscriber_push("open_training", message, 4, post[:id], user_id, post[:channel_id], user_push_to, user_push_id)
      ...
      response = Mession.tracked_subscriber_push("open_quiz", message, 4, post[:id], user_id, post[:channel_id], user_push_to, user_push_id)
