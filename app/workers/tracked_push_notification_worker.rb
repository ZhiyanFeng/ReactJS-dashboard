class TrackedPushNotificationWorker
  ...
  def perform(user,post_id,post_content,post_channel_id,cpr_id,post_archtype,base_type)
    response = 0
    if post_archtype
      message = "#{user[:first_name]} #{user[:last_name]} posted a shift trade request. Interested in helping out?"
      response = user.mession.tracked_subscriber_push("open_app", message, 4, post_id, user[:id], post_channel_id, user.mession[:push_to], user.mession[:push_id])
    elsif base_type == "announcement"
      message = user[:first_name] + " " + user[:last_name] + " announced: " + post_content
      response = user.mession.tracked_subscriber_push("open_detail", message, 4, post_id, user[:id], post_channel_id, user.mession[:push_to], user.mession[:push_id])
      ...
      response = user.mession.tracked_subscriber_push("open_training", message, 4, post_id, user[:id], post_channel_id, user.mession[:push_to], user.mession[:push_id])
      ...
      response = user.mession.tracked_subscriber_push("open_quiz", message, 4, post_id, user[:id], post_channel_id, user.mession[:push_to], user.mession[:push_id])
  ...
  def tracked_subscriber_push(action, message, source=nil, source_id=nil, user_object=nil, channel_id, push_to, push_id)
    ...
    if push_to == "GCM"
      begin
        ...
        ErrorLog.create(
          :file => "rb",
          :function => "user.mession.tracked_subscriber_push",
