class TrackedPushNotificationWorker
  ...
  def perform(user,post,cpr_id,post_archtype,base_type)
    ...
    if post_archtype
      ...
      response = Mession.tracked_subscriber_push("open_app", message, 4, post[:id], user[:id], post[:channel_id], user.mession[:push_to], user.mession[:push_id])
    elsif base_type == "announcement"
      message = user[:first_name] + " " + user[:last_name] + " announced: " + post[:content]
      response = Mession.tracked_subscriber_push("open_detail", message, 4, post[:id], user[:id], post[:channel_id], user.mession[:push_to], user.mession[:push_id])
      ...
      response = Mession.tracked_subscriber_push("open_training", message, 4, post[:id], user[:id], post[:channel_id], user.mession[:push_to], user.mession[:push_id])
      ...
      response = Mession.tracked_subscriber_push("open_quiz", message, 4, post[:id], user[:id], post[:channel_id], user.mession[:push_to], user.mession[:push_id])
