class TrackedPushNotificationWorker
  ...
  def perform(user,post,cpr_id,post_archtype)
    response = 0
    if post_archtype
      ...
      response = @mession.tracked_subscriber_push("open_training", message, 4, post[:id], user[:id], post[:channel_id])
      ...
      response = @mession.tracked_subscriber_push("open_quiz", message, 4, post[:id], user[:id], post[:channel_id])
