class Zencoder
  ...
  default_params :api_key => "83516b89da4a815187d505016afcfe96"
  format :json
  ...
  def initialize(encode_bucket_url = nil, notification_url = nil, thumbnail_url = nil)
  ...
  def encode(input_video, width, height, thumbnail_destination, options = {})
    ...
    query = {
      ...
      :outputs => { 
        ...
        :thumbnails => {
          :number => 1,
          :base_url => @thumbnail_url + thumbnail_destination,
          :size => "200x200"
        }
      }
    }.merge(options)
